...
class _LINE(object):
    ...
    def __init__(self, graph, rep_size=128, batch_size=1000, negative_ratio=5, order=3, hs=0):
        self.cur_epoch = 0
        self.order = order
        self.hs = hs
        self.g = graph
    ...
    def build_graph(self):
        ...
        cur_seed = random.getrandbits(32)
        self.embeddings = tf.get_variable(name="embeddings"+str(self.order)+str(self.hs), shape=[
                                          self.node_size, self.rep_size], initializer=tf.contrib.layers.xavier_initializer(uniform=False, seed=cur_seed))
        self.context_embeddings = tf.get_variable(name="context_embeddings"+str(self.order)+str(self.hs), shape=[
                                                  self.node_size, self.rep_size], initializer=tf.contrib.layers.xavier_initializer(uniform=False, seed=cur_seed))
...
class LINE(object):
    ...
    def __init__(self, graph, rep_size=128, batch_size=1000, epoch=10, negative_ratio=5, order=3, label_file=None, clf_ratio=0.5, auto_save=True, hs=0):
        self.rep_size = rep_size
        self.order = order
        self.hs = hs
        self.best_result = 0
        ...
        if order == 3:
            self.model1 = _LINE(graph, rep_size/2, batch_size,
                                negative_ratio, order=1, hs=self.hs)
            self.model2 = _LINE(graph, rep_size/2, batch_size,
                                negative_ratio, order=2, hs=self.hs)
            ...
            self.model = _LINE(graph, rep_size, batch_size,
                               negative_ratio, order=self.order, hs=self.hs)
            for i in range(epoch):
    ...
    def get_embeddings(self):
        ...
        self.vectors = {}
        if self.order == 3:
            vectors1, embeddings1 = self.model1.get_embeddings()
            vectors2, embeddings2 = self.model2.get_embeddings()
            for node in vectors1.keys():
                self.vectors[node] = np.append(vectors1[node], vectors2[node])
                self.embeddings = np.concatenate((embeddings1, embeddings2), axis=1)
        else:
            self.vectors, embeddings = self.model.get_embeddings()
